...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $sql = "UPDATE familles_topo SET materiel = ?, abv = ?, active = ?, observation = ? WHERE id = ?";
...
$sql = "SELECT * FROM familles_topo WHERE id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $id);
$stmt->execute();
$result = $stmt->get_result();
$abbreviation = $result->fetch_assoc();
?>
...
<head>
    ...
    <title>Modifier une abréviation</title>
    ...
    <?php if ($_SESSION['privilege'] === 'admin') { ?>
            ...
            <div class="right_col" role="main">
                <div class="form-container">
                    <h1>Modifier une abréviation</h1>
                    <form method="post">
                        <div class="form-group">
                            <label for="materiel">Matériel *</label>
                            <input type="text" id="materiel" name="materiel" required="required" class="form-control" value="<?php echo $abbreviation['materiel']; ?>">
                        </div>
                        <div class="form-group">
                            <label for="abv">Abbréviation *</label>
                            <input type="text" id="abv" name="abv" required="required" class="form-control" value="<?php echo $abbreviation['abv']; ?>">
                        </div>
                        <div class="form-group">
                            <label for="active">Active</label>
                            <input type="checkbox" id="active" name="active" class="form-control" <?php echo $abbreviation['active'] ? 'checked' : ''; ?>>
                        </div>
                        <div class="form-group">
                            <label for="observation">Observation</label>
                            <textarea id="observation" name="observation" class="form-control"><?php echo $abbreviation['observation']; ?></textarea>
                        </div>
